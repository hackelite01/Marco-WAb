...
export default class MessageHandler {
    ...
    handleMessage = async (M: ISimplifiedMessage): Promise<void> => {
            ...
            */
			...
			if (!command)
				return void M.reply(
					`No such command, Crazy! Have you never seen someone use the command *${this.client.config.prefix}help*.`
				);
			...
			if (command.config?.adminOnly && !M.sender.isAdmin)
				return void M.reply(`This command is only meant for the group admins, Crazy!`);
			try {
		...
		moderate = async (M: ISimplifiedMessage): Promise<void> => {
			...
			if (M.urls.length) {
				const groupinvites = M.urls.filter((url) =>
					url.includes("https://chat.whatsapp.com/")
				);
